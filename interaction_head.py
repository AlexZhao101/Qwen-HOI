...
class InteractionHead(Module):
    ...
    def __init__(self,
        ...
        self.box_feature_transformer = Transformer(width=1664, heads=16, layers=2)
        self.ln_post = LayerNorm(1664)

        decoder_layer = TransformerDecoderLayer(4096, 8, 2048, 0.1, "relu", False)
        decoder_norm = nn.LayerNorm(4096)
        self.decoder = TransformerDecoder(decoder_layer, 1, decoder_norm,
                                              return_intermediate=False)
        self.mlp = nn.Linear(1664, 4096)

        # self.word_embed = torch.load("obj_embed.pt")
    ...
    def forward(self,
        ...
        with torch.no_grad():
            avg_features, vit_features = self.backbone(images)
        ...
        if self.pose:
            ...
            human_joints_score = [None for _ in detections]
        if self.backbone_name == "resnet50" or self.backbone_name=="CLIP" or self.backbone_name == "GLIP":
        ...
        elif self.backbone_name == "Qwen-VL-ViT":
            # box_features, global_features = self.backbone.encode_image(images, box_coords, box_segs, need_patch=False)

            B, L, C = vit_features.permute(1, 0, 2).shape
            features['0'] = vit_features.permute(0,2,1).reshape(B, C, int(L**0.5), int(L**0.5)).to(dtype=torch.float32)
            features['3'] = avg_features.to(dtype=torch.float32)

            if box_coords is not None:
                res = self.box_feature_transformer(vit_features, 14, box_coords, pose_attention_weight=None,
                                                            need_patch=False)
            else:
                res = {}
                res['x'] = vit_features.permute(1, 0, 2)[:,1:,:]
                res['cls'] = vit_features.permute(1, 0, 2)[:, 0, :].unsqueeze(0)
            mask_cls_tok = res['x']
            cls_tok = res['cls']
            ...
            length = []
            for _ in box_labels:
                length.append(_.shape[0])

            bxfs = mask_cls_tok.split(length)
            res_decoder = []
            for i, bx_feature in enumerate(bxfs):
                res_decoder.append(
                    self.decoder(self.mlp(bx_feature), avg_features[i]).squeeze(0)
                )

            box_features = torch.cat(res_decoder)

            # box_features = mask_cls_tok  # (N, 1664)
            # gloabl feature (N,256,4096)
            # text_embed = self.word_embed[torch.cat(box_labels)]
            # fusion_features = torch.cat((box_features,text_embed),dim=1)

            features['global'] = cls_tok
            pose_box_features = None
        #为什么没有输出了
    ...
    def __init__(self,
        ...
        if self.backbone_name == "CLIP_CLS":
            ...
            self.box_head = nn.Sequential(
                ...
                nn.Linear(4096, node_encoding_size),
                # nn.Linear(1664, node_encoding_size),
                nn.ReLU(),
                nn.Linear(node_encoding_size, node_encoding_size),
                nn.ReLU()
            )
    ...
    def forward(self,
        ...
        for b_idx, (coords, labels, scores, human_joint, human_joint_score) in enumerate(zip(box_coords, box_labels, box_scores, human_joints, human_joints_score)):
            ...
            if self.pose:
                ...
                box_pair_spatial_semantic = self.spatial_head(box_pair_spatial_semantic)
