...
from CLIP.clip.model import convert_weights
...
torchvision Imagelist
    ...
    def __init__(self,
        # backbone: nn.Module,
        backbone_name: str, interaction_head: nn.Module,
    ...
    def preprocess(self,
        ...
        if self.backbone_name == "resnet50":
           ...
           detections, original_image_sizes, images.image_sizes
        ...
        elif self.backbone_name == "CLIP" or self.backbone_name == "CLIP_CLS" or self.backbone_name == "Qwen-VL-ViT":
            ...
            for img in images:
                img = topilimage(img)
                # if self.backbone_name == "Qwen-VL-ViT":
                img, trans, img_meta = warpaffine_image(img, n_px=448, device=device)
                # else:
...
class VIPLO(GenericHOINetwork):
    def __init__(self,
        ...
        #new
        elif backbone_name == "Qwen-VL-ViT":
            model_vit = visual.VisionTransformer(
                image_size=448,
                heads=16,
                layers=48,
                mlp_ratio=4.9231,
            ...
            model_vit.load_state_dict(torch.load(state_dict_path,map_location='cpu'))
            model_vit.float()
        ...
        box_pair_suppressor = nn.Linear(representation_size , 1)
        ...
        super().__init__(backbone_name, interaction_head, transform, postprocess, rank, patch_size, human_idx, pose)